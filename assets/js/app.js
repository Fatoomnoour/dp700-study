(() => {
  ...
  let state = loadState();
  // Reconcile previously saved multi-answer responses after answer-key updates.
      ...
      <section class="dump-list">
        ${pageItems.map(question => {
          ...
          return `<article class="dump-card" id="dump-${question.n}">
            <div class="dump-card__head"><div class="dump-card__head-group"><span class="dump-card__number">#${question.n}</span><span class="tag">${question.sourceFile ? PDF_SOURCE_LABELS[question.sourceFile] : `Run ${question.batch}`}</span><span class="tag">PDF page ${question.sourcePage || "?"}</span><span class="tag">${dumpTypeLabel(interaction)}</span></div><span class="tag">${escapeHtml(question.conceptArea || "DP-700")}</span></div>
